  ...
  const getOnSaleNftMetadata = async () => {
    try {
        ...
        for (let index = 1; index <= marketNftListArr.length ; index ++ ) {
          ...
          if( add === account ) { //해야될거 가져온 배열의 address들중에서 account와 같은 것만 추출.
            // console.log( 'push!' ) ;
    ...
    {dataURI?.map((v,i) =>{return<div>
        <Box className="mt-[82px] mb-[72px] lg:max-w-[800px] max-w-[460px]" key={i}>
          ...
          <Box className="flex flex-col justify-center items-center border rounded-md mb-10 ">
            ...
            <Box className="bg-gray-100 w-full px-4 py-1">
              ...
              <Button colorScheme="blue" onClick={handleCancel} className="justify-center text-center w-full py-4" >Cancel</Button>
              <Button colorScheme="blue" className="justify-center mt-1 text-center w-full py-4" >Withdraw Fundraise</Button>
            </Box>
          </Box>
        </Box>
    </div>
        ...
        {isCancelConfirmationOpen && (
          <Box className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50">
            <Box bg="white" p={4} rounded="md" shadow="md" maxW="sm" mx="auto" textAlign="center">
              <Text>Are you sure you want to cancel?</Text>
              <Box mt={4}>
                <Button colorScheme="blue" mr={2} onClick={handleConfirmCancel}>Confirm</Button>
                <Button colorScheme="teal" onClick={handleCancelCancel}>Cancel</Button>
              </Box>
            </Box>
          </Box>
        )}
