...
const MyNftCard = ({ tokenId, account }) => {
  ...
  const onClickApprove = async (e) => {
    ...
    try {
      setIsLoading(true);
      const checkApproved = await OGNFTContract.methods.getApproved(tokenData.edition).call({from : account});
    ...
    <Box className="flex flex-col justify-center items-center border rounded-md mb-10 ">
      {tokenData && <div>
        ...
        <Box className="bg-gray-100 w-full px-4 py-1">
          ...
          <div>{tokenData.attributes.map((v,i) => {return<div key={i}>{v.trait_type} {v.value}</div>})}</div>
          <Flex justify="center">
            <Button
              colorScheme="blue"
