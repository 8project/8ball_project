...
const FundingModal = ({ isOpen, onClose, tokenData, account, price, indexId}) => {
                        ...
                        <ModalBody>
                            ...
                            <Box className="mt-4 text-sm font-semibold">
                                <Box className="flex justify-between">
                                    ...
                                    <Text>{(buyer.length)*5}</Text>{/*진행률 rate 수정한 부분-조성윤*/}
                                </Box>
                            ...
                            <Box className="flex flex-col justify-center items-center text-xs mt-4">
                                ...
                                <div onClick={getFundingBuyerList}>   {/*수정한 부분-조성윤*/}
                                    <div className="text-lg bg-red-100">History</div>
                                    <div className="bg-blue-100">{buyer.map((v,i) => {return<div key={i}>{v}</div>})}</div>
                                </div>
                            </Box>
                        </ModalBody>
                    )}
