...
const OfferModal = ({ isOpen, onClose, offerMetadata, price, offerId, account }) => {
    ...
    const getOfferList = async () => {
        try {
            ...
            const responseBestOffer = await MarketContract.methods
                .trackingBestOffer(offerId)
                .call();
            setOfferAccount(responseGetOfferAccount);
            setOfferAmount(responseGetOfferAmount);

            setBestOffer(parseInt(responseBestOffer) / 10 ** 18);
    ...
    /*
    const getTrackingBestOffer = async () => {
        try {
            const response = await MarketContract.methods.trackingBestOffer(offerId).call()
